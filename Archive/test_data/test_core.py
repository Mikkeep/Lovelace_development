...
"""
def test_class(st_module, class_names, test_driver, init_args, ref_class, 
               init_kw={},
               lang="en",
               custom_msgs={},
               presenters=default_presenters,
               argument_cloner=default_argument_cloner,
               kwargs_cloner=default_argument_cloner,
               validate_exceptions=True):
               
    msgs = copy.deepcopy(default_class_test_msgs)
    msgs.update(custom_msgs)
    
    json_output.new_test(
        msgs.get_msg("ClassName", lang)["content"].format(name=class_names[lang])
    )
    ...
    try:
        st_class = getattr(st_module, class_names[lang])
    except NameError:
        sys.stdout = save_o
        sys.stderr = save_e
        output(msgs.get_msg("NoClass", lang), ERROR,
            name=class_names[lang]
        )
        return


    stored_args = argument_cloner(init_args)
    stored_kwargs = kwargs_cloner(init_kw)

    ref_inst = ref_class(*init_args, **init_kw)
    try:
        st_inst = st_class(*init_args, **init_kw)
    except Exception as e:
        sys.stdout = save_o
        sys.stderr = save_e
        etype, evalue, etrace = sys.exc_info()
        ...
        output(msgs.get_msg(ename, lang, default="GenericErrorMsg"), ERROR,
            call=call_presenter(
                class_names[lang], method_names[lang],
    ...
    for method_names, args, kwargs, validator, msg_key in test_driver(st_inst, ref_inst):
        stored_args = argument_cloner(args)
        stored_kwargs = kwargs_cloner(kw_args)
        
        output(msgs.get_msg(msg_key, lang, "TestTarget"), INFO, **ref_class.__dict__)
        
        ref_method = getattr(ref_inst, method_names[lang])
        try:
            ref = ref_method(*argument_cloner(args), **kwargs_cloner(kwargs))
        except Exception as e:
            if validate_exceptions:
                ref = e
            else:
                sys.stdout = save_o
                sys.stderr = save_e
        ...
        try:
            st_method = getattr(st_class, method_names[lang])
        except Exception as e:
            sys.stdout = save_o
            sys.stderr = save_e
            output(msgs.get_msg("NoMethod", lang), ERROR,
                name=class_names[lang]
            )
            continue
            
        try:
            res = st_method(*args, **kwargs)
        except Exception as e:
            if validate_exceptions:
                res = e
            else:
